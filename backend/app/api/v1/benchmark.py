...
async def process_single_bill(filename: str, model_name: str, ground_truth_data: dict, db: Session):
    ...
    if not bill:
        bill = Bill(filename=filename,
                    ground_truth_json=json.dumps(ground_truth_dict))
        db.add(bill)
        db.commit()
        db.refresh(bill)
    ...
    try:
        ...
        prediction, in_tokens, out_tokens = await adapter.extract_bill_data(image_path)
        latency = time.time() - start_time
        is_success = True
    except Exception as e:
        print(f"Failed extracting {filename} with {model_name}: {e}")
        prediction, in_tokens, out_tokens, latency, is_success = {}, 0, 0, 0.0, False
...
async def run_benchmark(db: Session = Depends(get_db)):
    ...
    if not os.path.exists(gt_file_path):
        raise HTTPException(
            status_code=400, detail="ground_truth.json not found in ground_truth directory.")
    ...
    if not os.path.exists(DATASET_DIR):
        raise HTTPException(
            status_code=400, detail=f"Dataset directory '{DATASET_DIR}' not found.")

    models_to_test = ["openai", "claude", "gemini"]
    processed_count = 0

    image_files = [f for f in os.listdir(
        DATASET_DIR) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
