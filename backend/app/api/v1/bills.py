...
# --- THESE TWO LINES MUST BE HERE FOR THE APP TO START ---
router = APIRouter()
...
async def extract_bill(
    ...
    """
    MODE 2: Live extraction from a single uploaded bill.
    Will attempt to hit the Live Gemini API (defaults to use_mock=False).
    """
    ...
    try:
        # Defaults to use_mock=False, forcing a live API call
...
async def evaluate_bill(
    ...
    Passes use_mock=True to Gemini to preserve API quota.
    """
    # 1. Save File Locally
    ...
    try:
        if model_name == "gemini":
            prediction, in_tokens, out_tokens = await adapter.extract_bill_data(file_path, use_mock=True)
        else:
            prediction, in_tokens, out_tokens = await adapter.extract_bill_data(file_path)
